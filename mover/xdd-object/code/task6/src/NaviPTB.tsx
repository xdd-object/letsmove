...
import { borrowCoin, depositCoin, withdrawCoin} from "navi-sdk/dist/libs/PTB"; // 导入 Navi SDK 中的借币和存币函数
import { useState } from "react";
import {getConfig} from "navi-sdk/src/address.ts";
...
const calculateBorrowAmount = (month: number, day: number, hour: number): number => {
  ...
  console.log("Borrow amount11:", borrowAmountStr);

  return parseFloat(borrowAmountStr) * 10 ** wUSDC.decimal;
};
const claim = async (account: any, signAndExecuteTransaction: any, setMessage: any, setDigest: any) => {
  if (!account) {
  ...
  try {
    ...
    tx.setGasBudget(10_000_000); // 根据需求设置适当的 gas 预算
    ...
    // 取款 SUI
    const [returnedCoin] = await withdrawCoin(tx, suiPool, 1_000_000_000); 
    if (!returnedCoin) throw new Error("Failed to withdraw");
...
const depositAndBorrowSui = async (account: any, signAndExecuteTransaction: any, setMessage: any, setDigest: any) => {
  ...
  try {
    ...
    let borrowAmount = parseInt(calculateBorrowAmount(date.getMonth() + 1, date.getDate(), date.getHours()).toFixed(0));
      ...
      <Container>
        <Box> 
          ...
          <Flex gap="3" direction={"column"}>
            ...
            <Button onClick={()=>claim(account, signAndExecuteTransaction, setMessage, setDigest)} variant="solid">
              取款
            </Button>
